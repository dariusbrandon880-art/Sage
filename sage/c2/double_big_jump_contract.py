...
- waves share the same anchored repository HEAD;
- missing/stale HEAD evidence fails closed;
...
class DoubleBigJumpWaveSpec:
    ...
    def validate(self) -> None:
        ...
        slots = tuple(m.slot_id for m in self.missions)
...
def validate_double_big_jump_waves(waves: Iterable[DoubleBigJumpWaveSpec]) -> tuple[DoubleBigJumpWaveSpec, DoubleBigJumpWaveSpec]:
    pair = tuple(waves)
    if len(pair) != 2:
...
def reconverge_double_big_jump(*, wave_results: dict[str, bool], waves: Iterable[DoubleBigJumpWaveSpec]) -> bool:
