...
Aggregates 5-flight execution receipts across the canonical 5x4 20-cell lifecycle matrix and emits an evidence-bound immersion projection.
"""
...
from sage.c2.immersion_projection import project_impact_stars
...
class ReconvergenceEvidencePackage(BaseModel):
    """Evidence package plus a read-only projection of verified impact."""
    wave_id: str
    ...
    immersion_projection: Dict[str, Any] = Field(default_factory=dict)
    timestamp: float = Field(default_factory=time.time)
    package_hash: str = ""
    ...
    def compute_hash(self) -> str:
        ...
        projection_blob = json.dumps(self.immersion_projection, sort_keys=True, separators=(",", ":"))
        payload = f"{self.wave_id}:{self.total_flights}:{self.successful_flights}:{self.first_pass_verification_rate}:{self.reconvergence_verdict}:{flights_blob}:{matrix_blob}:{projection_blob}:{self.timestamp}"
        return hashlib.sha256(payload.encode("utf-8")).hexdigest()


class C2ReconvergenceSynthesizer:
    """Synthesizer aggregating wave evidence and evaluating 5x4 readiness."""

    def __init__(self, wave_id: str):
        self.wave_id = wave_id

    def synthesize_reconvergence(self, flight_summaries: List[FlightExecutionSummary]) -> ReconvergenceEvidencePackage:
        total = len(flight_summaries)
        successful = 0
        blocked = 0
        advancement_matrix: Dict[str, bool] = {}
        sha_pattern = re.compile(r"^[0-9a-fA-F]{40}$")
        stages = [LifecycleStage.INTAKE_RECON, LifecycleStage.BOUNDED_BUILD, LifecycleStage.VERIFY_PROOF, LifecycleStage.WAREHOUSE_PROMOTE]
        ...
        for idx, flight in enumerate(flight_summaries, start=1):
            ...
            for s_idx, stage in enumerate(stages, start=1):
                advancement_matrix[f"{path_id}-S{s_idx}"] = any(m.stage == stage and m.passed for m in flight.lifecycle_milestones)
            is_success = (flight.execution_result.upper() in ("PASS", "SUCCESS", "EXECUTED") and is_valid_sha and is_active_or_ready and all_milestones_passed and flight.blocker is None)
            if is_success:
                successful += 1
            else:
                blocked += 1
        ...
        impact = project_impact_stars(verified_cells=sum(advancement_matrix.values()), total_cells=20, verdict=verdict)
        projection = {"type": "MILESTONE_STRIKE", "wave_id": self.wave_id, "verdict": verdict, "verified_cells": impact.verified_cells, "safe_impact_stars": impact.stars, "rank": impact.rank}

        pkg = ReconvergenceEvidencePackage(
            wave_id=self.wave_id, flight_summaries=flight_summaries, total_flights=total,
            successful_flights=successful, blocked_flights=blocked, advancement_matrix_20_cells=advancement_matrix,
            first_pass_verification_rate=rate, reconvergence_verdict=verdict, immersion_projection=projection,
        )
        pkg.package_hash = pkg.compute_hash()
        return pkg

    def get_matrix_stage_breakdown(self, package: ReconvergenceEvidencePackage) -> Dict[str, Dict[str, Any]]:
        stages = [LifecycleStage.INTAKE_RECON, LifecycleStage.BOUNDED_BUILD, LifecycleStage.VERIFY_PROOF, LifecycleStage.WAREHOUSE_PROMOTE]
        breakdown = {}
        ...
        for s_idx, stage in enumerate(stages, start=1):
            ...
            breakdown[stage.value] = {"stage": stage.value, "passed_count": passed_cells, "total_count": total_flights, "pass_rate": (passed_cells / total_flights) * 100.0}
        return breakdown
