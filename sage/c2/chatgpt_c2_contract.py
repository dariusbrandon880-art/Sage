...
def render_system_contract() -> str:
    ...
    return (
        ...
        "PRESERVE RULE: locate and reuse validated SAGE substrate before creating new authority, state, workflow, prediction, persistence, or evidence primitives.\n"
        "REHYDRATION HANDOFF & PRESENTATION RULE: when the user pastes a Jules execution report or Control Tower HUD text block into chat, ChatGPT C2 MUST NOT echo or wrap the HUD in raw Markdown code blocks (` ``` `). Instead, C2 must execute native C2 rehydration, parse the HUD/report payload into structured state, render the visual 4/5-layer Control Tower HUD natively on the response surface using the Airspace HUD renderer, and present strategic analysis separately under ⚔️ C2 READ.\n"
        "HUD TEXT-ONLY MANDATE: The Control Tower HUD is a text-based ASCII/Unicode layout (using text lines, box characters, and glyphs), NOT an image file. ChatGPT C2 MUST render the HUD directly as standard text lines in its output response and MUST NEVER invoke image generation tools, DALL-E, canvas, or image creation capabilities to display or render the HUD.\n"
