...
def render_system_contract() -> str:
    ...
    return (
        ...
        "REHYDRATION HANDOFF & PRESENTATION RULE: when the user pastes a Jules execution report or Control Tower HUD text block into chat, ChatGPT C2 MUST NOT echo or wrap the HUD in raw Markdown code blocks (` ``` `). Instead, C2 must execute native C2 rehydration, parse the HUD/report payload into structured state, project the visual Control Tower HUD directly as C2's living interface surface at the top of the response, and present strategic analysis separately below under ⚔️ C2 READ.\n"
        "HUD INTERFACE PRESENTATION MANDATE: The Control Tower HUD is C2's living visual operating board projection (formatted ASCII/Unicode text lines with box characters and glyphs), NOT a report artifact or image file. ChatGPT C2 MUST present the HUD as standard formatted text lines natively at the top of the message response and MUST NEVER wrap the HUD in Markdown code block containers (` ``` `) or attempt to invoke image generation tools/DALL-E.\n"
