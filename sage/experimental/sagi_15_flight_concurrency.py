...
# Mapping of roles to real test references for each session
REAL_TEST_MAPPING: Dict[int, Dict[FlightRole, tuple[str, str, list[str]]]] = {
    1: {
        FlightRole.CONSEQUENT_FRONTIER: (
            ...
            "sage.c2.frontier_intel_1",
            ["tests/c2/test_frontier_admission.py"],
        ),
        FlightRole.INFORMATION_GAIN: (
            "sage/experimental/sagi_discovery_flight_selector.py",
            "sage.experimental.selector_1",
            ["tests/experimental/test_sagi_discovery_flight_selector.py"],
        ),
        FlightRole.FALSIFICATION: (
            "sage/c2/flight_collision_lock.py",
            "sage.c2.collision_1",
            ["tests/c2/test_flight_collision_lock.py"],
        ),
        ...
        FlightRole.INDEPENDENT_TRANSFER: (
            "sage/c2/reconvergence_synthesizer.py",
            "sage.c2.reconvergence_1",
            ["tests/c2/test_reconvergence_synthesizer.py"],
        ),
    },
    2: {
        FlightRole.CONSEQUENT_FRONTIER: (
            "sage/c2/workflow_velocity.py",
            "sage.c2.velocity_2",
            ["tests/c2/test_workflow_velocity.py"],
        ),
        FlightRole.INFORMATION_GAIN: (
            "sage/experimental/sagi_flight_wave.py",
            "sage.experimental.wave_2",
            ["tests/experimental/test_sagi_flight_wave.py"],
        ),
        FlightRole.FALSIFICATION: (
        ...
        FlightRole.RECOVERY_REGRESSION: (
            "sage/c2/flight_gps/engine.py",
            "sage.c2.gps_2",
            ["tests/c2/test_flight_gps.py"],
        ),
        FlightRole.INDEPENDENT_TRANSFER: (
            "sage/c2/organism_jigsaw.py",
            "sage.c2.jigsaw_2",
            ["tests/c2/test_organism_jigsaw.py"],
        ),
    },
    3: {
        FlightRole.CONSEQUENT_FRONTIER: (
            "sage/runtime/engine.py",
            "sage.runtime.engine_3",
            ["tests/test_system_frame.py"],
        ),
        FlightRole.INFORMATION_GAIN: (
            "sage/experimental/sagi/sagi.py",
            "sage.experimental.sagi_3",
            ["tests/experimental/test_sagi_simulator.py"],
        ),
        FlightRole.FALSIFICATION: (
            "sage/c2/c2_execution_surface.py",
            "sage.c2.surface_3",
            ["tests/c2/test_c2_execution_surface.py"],
        ),
        FlightRole.RECOVERY_REGRESSION: (
            "sage/experimental/airspace/fleet_concurrency.py",
            "sage.experimental.concurrency_3",
            ["tests/experimental/test_fleet_concurrency.py"],
        ),
        FlightRole.INDEPENDENT_TRANSFER: (
...
def check_timing_concurrency(details: List[FlightExecutionDetail]) -> bool:
    """Verify thread and timing overlap across executed flights.

    Returns True if at least two flights executed on different worker threads
    with overlapping [start_time, end_time] intervals.
    """
    distinct_threads = {d.worker_thread for d in details}
    if len(distinct_threads) < 2:
        return False

    # Check pairwise interval overlap for flights on distinct threads
    for i in range(len(details)):
        for j in range(i + 1, len(details)):
            d1, d2 = details[i], details[j]
            if d1.worker_thread != d2.worker_thread:
                # Two intervals [s1, e1] and [s2, e2] overlap iff max(s1, s2) <= min(e1, e2)
                if max(d1.start_time, d2.start_time) <= min(d1.end_time, d2.end_time):
                    return True
    return False


class SAGI15FlightConcurrencyEngine:
    """Orchestrates 3 concurrent Jules sessions running 5 parallel flights each (15 total).
    ...
    Enforces anti-collision flight locking, exact git HEAD SHA binding, real test execution,
    and timing overlap verification.
    """
    ...
    def _execute_single_flight(
        ...
        exact_git_head: str,
        wave_id: str,
    ) -> tuple[FlightExecutionDetail, FlightExecutionSummary]:
        start_time = time.time()
        worker_thread = threading.current_thread().name

        role = candidate.role
        role_mapping = REAL_TEST_MAPPING[sess_idx][role]
        target_path, collision_ns, test_refs = role_mapping
        ...
        with self._lock_guard:
            lock_req = FlightLockRequest(
                ...
                target_namespaces=[collision_ns],
            )
        ...
        try:
            if test_refs and lock_res.acquired:
                # Execute real pytest command
                proc = subprocess.run(
                    [sys.executable, "-m", "pytest", *test_refs],
                    capture_output=True,
                    text=True,
                )
            ...
            detail = FlightExecutionDetail(
                session_id=sess_id,
                flight_id=flight_id,
                role=role.value,
                candidate_id=candidate.candidate_id,
                target_path=target_path,
                collision_namespace=collision_ns,
                start_time=start_time,
                end_time=end_time,
                worker_thread=worker_thread,
                lock_acquired=lock_res.acquired,
                tests_passed=tests_passed,
                test_references=test_refs,
                execution_result=exec_result,
            )

            # Milestones for 20-cell matrix
    ...
    def _execute_session_wave(
        ...
        exact_git_head: str,
        wave_id: str,
        flight_executor: ThreadPoolExecutor,
        ...
        # Sort flight details and summaries by flight_id
        flight_details.sort(key=lambda d: d.flight_id)
        flight_summaries.sort(key=lambda s: s.flight_id)

        # Synthesize 20-cell advancement matrix for session
    ...
    def execute_concurrency_wave(
        ...
        """Executes a 15-flight concurrency wave across 3 Jules sessions in parallel."""
        ...
        # Execute 3 session waves concurrently using dedicated session and flight thread pools
        with ThreadPoolExecutor(max_workers=self.max_session_workers, thread_name_prefix="sagi-sess") as session_executor, \
             ThreadPoolExecutor(max_workers=self.max_flight_workers, thread_name_prefix="sagi-flt") as flight_executor:
            session_futures = {
                session_executor.submit(
                    self._execute_session_wave,
                    sess_idx,
                    sess_id,
                    session_candidates[sess_id],
                    exact_git_head,
                    wave_id,
                    flight_executor,
                ): sess_id
        ...
        total_cells_advanced = sum(
            sum(1 for v in s.advancement_matrix_20_cells.values() if v)
            for s in session_summaries.values()
        )

        is_concurrent = check_timing_concurrency(all_flight_details)

        rolls_royce_passed = (
            total_successful_flights == 15
        ...
        receipt = SAGI15FlightConcurrencyReceipt(
            ...
            is_truly_concurrent=is_concurrent,
            session_summaries={s_id: session_summaries[s_id] for s_id in target_sessions},
        )
