...
class MarketSnapshot:
    ...
    def canonical_market_type(self) -> str:
        """Return explicit market taxonomy, falling back to the legacy market name."""
        return (self.market_type or self.market).strip().lower()

    @property
    def canonical_line_value(self) -> str:
        """Stable textual representation used by prediction identity surfaces."""
        if self.line_value is None:
            return ""
        return format(self.line_value, ".12g")

    @property
    def market_identity(self) -> tuple[str, str, str, str]:
        """Identity dimensions that distinguish market angles on one event."""
...
class PlayerPropSnapshot:
    ...
    def canonical_line_value(self) -> str:
        if self.threshold is None:
            return ""
        return format(self.threshold, ".12g")

    @property
    def market_identity(self) -> tuple[str, str, str, str, str]:
        return (
            self.event_id,
            self.canonical_market_type,
            self.prop_category.strip().lower(),
            self.canonical_line_value,
...
class FanDuelSnapshotAdapter:
    ...
    def from_mapping(cls, payload: Mapping[str, Any]) -> MarketSnapshot:
        ...
        raw_line = market.get("line_value")
        if raw_line is None:
            raw_line = payload.get("line_value")
