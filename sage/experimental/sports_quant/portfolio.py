...
The portfolio engine expands observation volume without crossing the zero-wagering
boundary. It treats a "bet" as a distinct paper prediction record and keeps
parlay parents separate from their independently scored legs.
"""
...
class DailyPortfolio:
    """Immutable daily paper-prediction portfolio and construction diagnostics."""
    ...
    def target_met(self) -> bool:
        return self.count >= DEFAULT_DAILY_TARGET
...
class DailySportsPortfolioEngine:
    ...
    def _identity(record: PredictionRecord) -> tuple[str, str, str, str, str]:
        """Canonical prediction identity; identical observations cannot count twice."""
        return (
            record.event_id,
            record.market.strip().lower(),
    ...
    def _build_parlays(
        ...
        # One leg per event prevents the same underlying game from dominating a
        # cross-game parlay. Same-game parlays can be built separately from props.
        ...
        for leg_count in range(self.min_parlay_legs, self.max_parlay_legs + 1):
            ...
            for combo in combinations(event_representatives, leg_count):
                ...
                parent_id = f"daily-{abs(hash(parent_material)):x}"
    ...
    def build(
        ...
        target_parlays = min(
            int(round(self.target * self.parlay_share)),
            max(0, self.target - 1),
        )
        parlays = self._build_parlays(singles, target_parlays)

        # Fill the remaining daily target with unique singles. If the supplied
        # market universe cannot support the requested target, fail closed rather
        # than fabricate predictions.
        ...
        # Re-check the complete portfolio identity. Parlay parents are intentionally
        # distinct from their legs because is_parlay participates in their identity.
        ...
        single_count = sum(not record.is_parlay for record in records)
        parlay_count = sum(record.is_parlay for record in records)
