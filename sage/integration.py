...
class GoogleWorkspaceSyncManager:
    """Compatibility-preserving Google Workspace synchronization manager."""
...
class GoogleDriveProjectionSyncManager:
    """Persistent continuity projection manager with readback conflict detection."""
    ...
    def sync_projection_to_drive(self, credentials_path: str | None = None, target_dir: str = "SAGE") -> dict[str, Any]:
        ...
        try:
            try:
                creds = service_account.Credentials.from_service_account_file(str(cred_path), scopes=["https://www.googleapis.com/auth/drive.file"])
            except Exception:
            ...
            if folders:
                ...
                folder_id = files_api.create(body={"name": "SAGE", "mimeType": "application/vnd.google-apps.folder"}, fields="id").execute().get("id")
            live_synced_files = []
            for item in synced_files:
                ...
                if files:
                    file_id = files_api.update(fileId=files[0]["id"], media_body=media, fields="id").execute().get("id")
                    action = "updated"
                else:
                    file_id = files_api.create(body={"name": item["filename"], "parents": [folder_id]}, media_body=media, fields="id").execute().get("id")
                    action = "created"
            ...
            return {"mode": "live", "status": "failed", "error": str(e), "stale_conflict_check": {"local_head_sha": local_head, "remote_head_sha": "unknown", "status": "ERROR"}, "is_valid": False}
