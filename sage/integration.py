...
class ChatGPTClient(BaseAIClient):
    ...
    def create_interface_transport(self, session_id: str, command_authorizer: Any = None):
        ...
        def _immersion_projector(s_id: str):
            ...
            return build_chatgpt_immersion_state(
                self.runtime, session_id=s_id, c2_context=c2_context
            )
    ...
    def _rehydrate_c2_context(self, session_id: str) -> dict[str, Any]:
        ...
        if self.c2_provider and callable(self.c2_provider):
            ...
            obj = status.get("current_objective") or getattr(
                getattr(self.runtime, "current_state", None), "current_objective", None
            )
            tsk = status.get("active_task") or getattr(
                getattr(self.runtime, "current_state", None), "active_task", None
            )
            base_context = {
                "c2_identity": "ChatGPT",
            ...
            try:
                ...
                if reports:
                    session_reports = [
                        ...
                        or (
                            isinstance(getattr(r, "content", None), dict)
                            and r.content.get("session_id") == session_id
                        )
                    ]
    ...
    def _build_governed_runtime(
        self, *, session_id: str, c2_context: dict[str, Any], evidence_refs: tuple[str, ...]
    ):
        from hashlib import sha256
        from sage.runtime.model_gateway import SAGERuntime, SAGEStateSnapshot

        objective = c2_context.get("active_objective") or getattr(
        ...
        snapshot = SAGEStateSnapshot(
            ...
            mission_id=str(
                c2_context.get("mission_id")
                or sha256(str(objective or session_id).encode("utf-8")).hexdigest()
            ),
        ...
        return SAGERuntime(
            snapshot, policy_version=str(c2_context.get("policy_version") or "sage-runtime-v1")
        )
    ...
    def execute_query(self, request: AIQueryRequest) -> AIQueryResponse:
        ...
        session_id = (
            request.session_id
            or getattr(getattr(self.runtime, "context", None), "session_id", None)
        ...
        governed_runtime = self._build_governed_runtime(
            session_id=session_id, c2_context=c2_context, evidence_refs=referenced_ids
        )
        if request.response_override:
            ...
            self.reasoning_history.append(
                f"ChatGPT boundary used a governed response override for session '{session_id}'."
            )
        else:
            ...
            self.reasoning_history.append(
                f"ChatGPT executed through the governed OpenAI adapter for session '{session_id}'."
            )
        ...
        immersion_state = build_chatgpt_immersion_state(
            self.runtime, session_id=session_id, c2_context=c2_context, evidence_refs=referenced_ids
        )
        ...
        rendered_immersion, model_response = SAGEChatGPTBoundary(
            governed_runtime, adapter, operational_runtime=self.runtime
        ).respond(
            request.prompt,
            model_role="chatgpt",
        ...
        payload = ExternalSessionPayload(
            ...
            objective=str(
                ...
                or self.runtime.current_state.current_objective
                or "SAGE Runtime Standby"
            ),
...
class GeminiJulesClient(BaseAIClient):
    ...
    def execute_query(self, request: AIQueryRequest) -> AIQueryResponse:
        ...
        self.reasoning_history.append(
            f"Gemini/Jules rehydrated C2 context for session '{session_id}' and aligned with {len(referenced_ids)} SAGE knowledge artifacts."
        )
        ...
        structured = SAGEProtocolGovernor.validate_and_parse(
            str(response_text), required_station="[SAGE::C2::GEMINI_JULES]"
        )
        if structured.violations:
            raise RuntimeError(
                f"SAGE Protocol Governance Violation: {'; '.join(structured.violations)}"
            )
        from sage.models import ExternalSessionPayload
...
class GoogleDriveProjectionSyncManager:
    ...
    def calculate_sha256(self, filepath: Path) -> str:
        ...
        return (
            ...
            else hashlib.sha256(filepath.read_bytes()).hexdigest()
        )
    ...
    def sync_projection_to_drive(
            ...
            synced_files.append(
                {
                    ...
                    "character_count": len(filepath.read_text()) if exists else 0,
                    "local_sha256": self.calculate_sha256(filepath),
                }
            )
        ...
        try:
            ...
            folders = (
                files_api.list(
                    q="name = 'SAGE' and mimeType = 'application/vnd.google-apps.folder' and trashed = false",
                    spaces="drive",
            ...
            folder_id = (
                folders[0]["id"]
                if folders
                else files_api.create(
                    body={"name": "SAGE", "mimeType": "application/vnd.google-apps.folder"},
                    fields="id",
                )
                .execute()
                .get("id")
            ...
            for item in synced_files:
                ...
                files = (
                    files_api.list(
                        q=f"name = '{item['filename']}' and '{folder_id}' in parents and trashed = false",
                        spaces="drive",
                        fields="files(id, name)",
                    )
                    .execute()
                ...
                if files:
                    file_id = (
                        files_api.update(fileId=files[0]["id"], media_body=media, fields="id")
                        .execute()
                        .get("id")
                    )
                    action = "updated"
                    ...
                    file_id = (
                        files_api.create(
                            body={"name": item["filename"], "parents": [folder_id]},
                            media_body=media,
                            fields="id",
                        )
                        .execute()
                ...
                live_synced_files.append(
                    {"filename": item["filename"], "file_id": file_id, "action": action}
                )
            remote_head = "unknown"
            aw_files = (
                files_api.list(
                    q=f"name = '05_ACTIVE_WORK.md' and '{folder_id}' in parents and trashed = false",
                    spaces="drive",
                    fields="files(id)",
                )
                .execute()
            ...
            if aw_files:
                remote_content = (
                    files_api.get_media(fileId=aw_files[0]["id"])
                    .execute()
            ...
            stale_status = (
                "SYNCHRONIZED"
                if remote_head in ("unknown", local_head)
                else "STALE / CONFLICTED PROJECTION"
            )
            return {
                "mode": "live",
                "status": "success",
                ...
                "is_valid": stale_status == "SYNCHRONIZED",
            }
