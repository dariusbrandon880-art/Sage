...
def _validate_receipt_payload(
    ...
    event_id = payload["acknowledged_event_id"]
    ...
    target = next((event for event in ledger_events if event.get("event_id") == event_id), None)
    if target is None:
        raise ValueError(f"Target event {event_id} does not exist in ledger")
