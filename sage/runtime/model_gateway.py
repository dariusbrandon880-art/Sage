...
class SAGERuntimeEnvelope:
    ...
    def _policy_digest(*, station: str, model_role: str, policy_version: str) -> str:
        payload = json.dumps(
            {"station": station, "model_role": model_role, "policy_version": policy_version},
            sort_keys=True,
            separators=(",", ":"),
        )
        return sha256(payload.encode("utf-8")).hexdigest()
    ...
    def from_state(
        ...
        *,
        ...
        return cls(
            ...
            policy_digest=cls._policy_digest(
                station=station,
                model_role=model_role,
                policy_version=policy_version,
            ),
...
class SAGEProtocolGovernor:
    ...
    ROLEPLAY_INDICATORS = (
        "as an ai", "in roleplay mode", "pretend that", "pretend you",
        "let's pretend", "imagine i am", "i will act as", "simulation mode",
        "virtual assistant persona", "character mode", "*nods*", "*smiles*", "*chuckles*",
    )
    AUTHORITY_CLAIM_INDICATORS = (
        "i hereby authorize", "i have updated canonical state", "update canonical state",
        "state mutated directly", "granting execution permissions", "bypassing preflight check",
        "overriding spek governance",
    )
    EVIDENCE_BYPASS_INDICATORS = (
        "ignore the evidence requirement", "ignore evidence requirement",
        "bypass evidence requirement", "without evidence requirement", "skip evidence validation",
    )
    UNVERIFIED_REPOSITORY_INDICATORS = (
        "claim a github change happened", "commit pushed to origin", "pushed commit to github",
    ...
    def validate_and_parse(cls, raw_output: str, required_station: str = "[SAGE::C2::CHATGPT]") -> SAGEStructuredResponse:
        ...
        import re
        station_matches = re.findall(r"\[SAGE::[^\]]+\]", raw_output)
        for match in station_matches:
            if match != required_station:
                violations.append(
                    f"Model output station identity mismatch: expected {required_station}, got {match}."
                )
            ...
            if isinstance(parsed_data, dict):
                ...
                for act in parsed_data.get("proposed_actions", []):
                    if isinstance(act, dict):
                        proposed_actions.append(SAGEActionProposal(
                ...
                if any(act.action_type.lower() in completion_action_types for act in proposed_actions) and not evidence_refs:
...
class SAGERuntime:
    ...
    def envelope(self, model_role: str, *, station: str = "[SAGE::C2::CHATGPT]") -> SAGERuntimeEnvelope:
        return SAGERuntimeEnvelope.from_state(self.state, model_role=model_role, station=station)

    def reconcile(self, response: ModelResponse, *, expected_station: str, model_role: str) -> None:
        ...
        if response.policy_version != "sage-runtime-v1":
            raise ValueError("SAGE policy version mismatch")
        expected_policy_digest = SAGERuntimeEnvelope._policy_digest(
        ...
        expected_provenance_digest = SAGERuntimeEnvelope._provenance_digest(self.state)
        if response.provenance_digest != expected_provenance_digest:
