...
COORDINATION_EVENT_TYPES = {
    "MISSION_CREATED", "SORTIE_CREATED", "SORTIE_TRANSITIONED", "INTEL_RECORDED",
    "QUALIFICATION_PROMOTED", "QUALIFICATION_CHALLENGED", "XP_AWARDED",
    "AGENT_COORDINATION_MESSAGE", "AGENT_HANDOFF", "AGENT_ASSIGNMENT",
    "AGENT_CHALLENGE", "AGENT_VERIFICATION", AGENT_COORDINATION_RECEIPT,
}

DELIVERABLE_COORDINATION_EVENT_TYPES = {
    "AGENT_COORDINATION_MESSAGE", "AGENT_HANDOFF", "AGENT_ASSIGNMENT",
    "AGENT_CHALLENGE", "AGENT_VERIFICATION",
}
...
def _events(manager_module: Any) -> list[dict[str, Any]]:
    return list(manager_module.AirspaceManager()._load_raw_events())
...
def _station_activity(station_id: Any, sorties: list[Any]) -> str:
    ...
    if not station_sorties:
        return STANDBY
    status = station_sorties[-1].status.value
...
def _communication_projection(events: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for event in events:
        ...
        projected.append({
            ...
            "recipients": _recipients(event), "participants": [str(v) for v in participants],
            "mission_id": event.get("mission_id"), "sortie_id": event.get("sortie_id"),
            "evidence_refs": list(event.get("evidence_refs", [])),
        })
    return projected[-10:]


def _identity_for_actor(state: Any, actor: str, *, state_label: str = STANDBY) -> dict[str, Any] | None:
    """Resolve an event actor to canonical identity without mutating state."""
    nameplate = importlib.import_module("sage.experimental.airspace.nameplate")
    for station_id, station in state.stations.items():
        identity = nameplate.build_agent_identity(state, station_id, state_label=state_label)
        aliases = {station_id.value, station.agent_name, identity["nameplate"]}
        if actor in aliases:
            return identity
    return None
...
def get_unread_coordination(agent_id: str) -> list[dict[str, Any]]:
    ...
    acknowledged = {
        ...
        for event in events
        if event.get("event_type") == AGENT_COORDINATION_RECEIPT and event.get("actor") == agent_id
    }

    unread = []
    for event in events:
        event_id = event.get("event_id")
        if event.get("event_type") not in DELIVERABLE_COORDINATION_EVENT_TYPES:
            continue
        if agent_id not in _recipients(event) or event_id in acknowledged:
            continue
        actor = str(event.get("actor") or "")
        sender_state = STANDBY
        for station_id, station in state.stations.items():
            aliases = {station_id.value, station.agent_name}
            if actor in aliases:
...
def get_coordination_state() -> dict[str, Any]:
    ...
    for station_id, station in state.stations.items():
        ...
        stations[station_id.value] = {
            "agent_name": station.agent_name, "activity": activity,
            ...
            "cql": station.current_cql, "sql": station.current_sql,
    ...
    return {
        ...
        "stations": stations,
        "active_sorties": [
            {"sortie_id": s.sortie_id, "mission_id": s.mission_id, "station": s.station.value,
             "objective": s.objective, "status": s.status.value}
            for s in sorties
        ],
...
def render_coordination_status() -> str:
    ...
    participants = [
        f"{station['agent_name']}:{station['activity']}"
        for station in context["stations"].values() if station["activity"] != STANDBY
    ]
