...
def _validate_evidence_refs(refs:list[str])->tuple[str,...]:
    ...
    return tuple(_require_text(ref,"evidence_ref") for ref in refs)
@dataclass(frozen=True)
class DecisionRecord:
    decision_id:str; context_id:str; authority_ref:str; evidence_refs:tuple[str,...]; decision_payload:Mapping[str,Any]; timestamp_locked:float|str; resolution:Mapping[str,Any]|None=None; capability_impact_ref:str|None=None; decision_hash:str=""; version:str=DECISION_RECORD_VERSION
    @classmethod
    def create(cls,*,decision_id:str,context_id:str,authority_ref:str,evidence_refs:list[str],decision_payload:Mapping[str,Any],timestamp_locked:float|str,envelope:Mapping[str,Any]|None=None)->"DecisionRecord":
        _require_text(decision_id,"decision_id"); _require_text(context_id,"context_id"); _require_text(authority_ref,"authority_ref"); refs=_validate_evidence_refs(evidence_refs)
        ...
        payload=_freeze(decision_payload); block={"decision_id":decision_id,"context_id":context_id,"authority_ref":authority_ref,"evidence_refs":refs,"decision_payload":payload,"timestamp_locked":timestamp_locked,"version":DECISION_RECORD_VERSION}; digest=hashlib.sha256(_canonical(block).encode()).hexdigest()
    ...
    def resolve(self,outcome:Mapping[str,Any],*,verification_status:str)->"DecisionRecord":
        ...
        _require_text(verification_status,"verification_status"); return replace(self,resolution=_freeze({**dict(outcome),"verification_status":verification_status}))
