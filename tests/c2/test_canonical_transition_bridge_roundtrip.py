...
def test_real_runtime_transition_persists_and_rehydrates(tmp_path):
    ...
    result = CanonicalC2TransitionBridge(runtime).apply(_response("prove real runtime state survives the next turn"))
    ...
    immersion = build_chatgpt_immersion_state(fresh_runtime, session_id="roundtrip-session", evidence_refs=result.evidence_refs)
