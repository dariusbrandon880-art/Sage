...
Covers:
1. POSITIVE: Valid GPT/model response enters SAGE, passes governance, preserves canonical state, passes response contract, reaches immersion rendering.
2. NEGATIVE / BYPASS: Response attempting to bypass SAGE governance or evidence rules is rejected.
3. WRONG STATION: Response claiming an invalid/wrong station identity is rejected.
4. GOVERNANCE FAILURE: Response violating the canonical governance contract fails closed.
5. AUTHORITY BOUNDARY: Model output remains proposal / evidence / data and never becomes SAGE authority.
6. STATE INTEGRITY: Rendering cannot manufacture canonical progression, authorization, evidence, or mission state.
"""
...
def test_authority_boundary_model_output_cannot_grant_authority(monkeypatch, tmp_path):
    ...
    assert status.get("active_task") is None
...
def test_state_integrity_rendering_cannot_manufacture_state(tmp_path):
    ...
    response = client.execute_query(
        AIQueryRequest(prompt="Render test", response_override="Valid response")
    )
