...
def mission(slot: str, name: str) -> FlightMissionSpec:
...
def wave(wave_id: str, prefix: str) -> DoubleBigJumpWaveSpec:
    return DoubleBigJumpWaveSpec(
        wave_id=wave_id,
        missions=tuple(mission(f"F{i}", f"{prefix}_{i}") for i in range(1, 6)),
    )


def test_two_waves_require_reusable_slots_and_unique_missions():
    waves = validate_double_big_jump_waves((wave("A", "alpha"), wave("B", "beta")))
...
def test_duplicate_wave_identity_fails_closed():
    with pytest.raises(ValueError, match="wave identities must be unique"):
        validate_double_big_jump_waves((wave("A", "alpha"), wave("A", "beta")))
...
def test_reconvergence_requires_both_independent_passes():
    waves = (wave("A", "alpha"), wave("B", "beta"))
