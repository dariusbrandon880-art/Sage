...
def test_experiment_ledger_no_self_promotion_on_receipt(tmp_path: Path) -> None:
    """Verify record_flight_receipt attaches evidence but maintains HOLD status without self-promoting."""
    ledger_file = tmp_path / "test_no_self_promote.json"
    ledger = ExperimentLedger(ledger_path=str(ledger_file))
    ...
    # Must NOT be PROMOTED automatically! Must remain HOLD.
    assert rec.status == ValidationStatus.HOLD
    assert "Awaiting formal validation decision" in rec.validation_notes
...
def test_build_jump_wave_fails_closed_on_ledger_exception(tmp_path: Path) -> None:
    """Verify BuildJumpWaveEngine fails closed if ExperimentLedger throws an exception."""
    broken_ledger = MagicMock(spec=ExperimentLedger)
    broken_ledger.record_flight_receipt.side_effect = RuntimeError("Ledger storage corrupt")
