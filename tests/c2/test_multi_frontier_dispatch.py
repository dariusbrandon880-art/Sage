...
def test_dispatch_requires_explicit_mission_assignments():
    with pytest.raises(ValueError, match="explicit current-wave mission assignments"):
        MultiFrontierDispatcher(engine_factory=lambda: FakeEngine(_package())).dispatch_all()


def test_dispatch_delegates_to_canonical_wave_engine():
    missions = _missions()
    receipt = MultiFrontierDispatcher(engine_factory=lambda: FakeEngine(_package()), missions=missions).dispatch_all()
    ...
    assert [r.mission_id for r in receipt.flight_receipts] == [f"mission-{i}" for i in range(1, 6)]


def test_dispatch_refuses_stale_execution_sha_per_flight():
    missions = _missions()
    receipt = MultiFrontierDispatcher(commit_sha="a" * 40, engine_factory=lambda: FakeEngine(_package(sha="b" * 40)), missions=missions).dispatch_all()
...
def test_dispatch_refuses_failed_wave():
    receipt = MultiFrontierDispatcher(engine_factory=lambda: FakeEngine(_package(passed=False)), missions=_missions()).dispatch_all()
    assert receipt.wave_verdict == "HOLD"
    assert all(r.status == "FAIL" for r in receipt.flight_receipts)
...
def test_serialization_contains_real_execution_metadata():
    receipt = MultiFrontierDispatcher(engine_factory=lambda: FakeEngine(_package()), missions=_missions()).dispatch_all()
