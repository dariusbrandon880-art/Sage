...
def test_outcome_regression_fails_closed(bridge, test_setup):
    ...
    # SAGE observations with a regression detected
    regressed_sage_obs = [
        FlightObservation(
            system="sage",
            mission_id="m_001_continuity",
...
def test_benchmark_invariant_violation_fails_closed(bridge, test_setup):
    ...
    # Violation: intervention timestamp before baseline timestamp
...
def test_one_way_import_law_ast_compliance():
    """Verify that sage/c2/governed_continuity_outcome_bridge.py contains zero static imports from sage.experimental."""
        ...
        if isinstance(node, ast.Import):
            for alias in node.names:
                assert "sage.experimental" not in alias.name, (
                    f"One-Way Import Law Violation: static import of '{alias.name}'"
                )
        elif isinstance(node, ast.ImportFrom):
            if node.module:
                assert "sage.experimental" not in node.module, (
                    f"One-Way Import Law Violation: static import from '{node.module}'"
                )
