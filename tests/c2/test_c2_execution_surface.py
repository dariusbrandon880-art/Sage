...
def test_success_and_receipt():
    sha = runtime_sha()
    rcpt = C2ExecutionSurfaceEngine().execute_request(
        C2ExecutionRequest(
            request_id="req-001",
            command_type=C2CommandType.READ,
            target_path="sage/c2/c2_execution_surface.py",
            starting_git_head=sha,
        )
    )
    assert rcpt.status == "EXECUTED" and len(rcpt.receipt_hash) == 64
    assert rcpt.starting_git_head == sha
    assert rcpt.resulting_git_head == sha


def test_stale_head_rejected():
    sha = runtime_sha()
    stale_sha = "0" * 40 if sha.lower() != "0" * 40 else "1" * 40
    rcpt = C2ExecutionSurfaceEngine().execute_request(
        C2ExecutionRequest(
            request_id="req-002",
            command_type=C2CommandType.READ,
            target_path="sage/c2/c2_execution_surface.py",
            starting_git_head=stale_sha,
        )
    )
