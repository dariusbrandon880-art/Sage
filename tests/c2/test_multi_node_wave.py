...
def test_single_node_optional_topology_success(wave_engine):
    nodes = {
        "NODE_SINGLE": {
            "role": NodeRole.PRIMARY_REPAIR.value,
            "flights": [
                {
                    "flight_id": f"S_F{i}",
                    "frontier_name": f"Frontier S{i}",
                    "target_namespaces": [f"sage/single_module_{i}/"],
                    "is_reserve": False,
                }
                for i in range(1, 6)
            ],
    ...
    assert receipt.reconvergence_verdict == "PASS"
    assert receipt.flow_anti_drift_verified is True
    assert receipt.total_flights_executed == 5
    assert len(receipt.node_results) == 1
