...
def test_continuity_bridge_ingest_pipeline(temp_workspace):
    ...
    payload_data = {
        ...
        "memories": [
            {"id": "mem_rule_001", "object_type": "rule", "content": {"title": "Ingestion Rule", "description": "All payloads must follow the single authoritative path", "archive": True}, "tags": ["architecture", "core"], "confidence": "validated"},
            {"id": "mem_fact_002", "object_type": "fact", "content": {"description": "Verification uses standard pytest-based test suites"}, "tags": ["testing"], "confidence": "hypothesis"},
        ],
        "decisions": [{"id": "dec_arch_001", "decision_type": "architectural", "description": "Establish standard bridge pathway", "rationale": "Enforces complete rehydration and validation rules with zero bypasses", "evidence": ["mem_rule_001"]}],
        "metadata": {"source": "ci_pipeline"},
    }
    result = runtime.ingest_session_payload(ExternalSessionPayload(**payload_data))
    ...
    assert (temp_workspace / f"{result['checkpoint_id']}.json").exists()
...
def test_continuity_reasoning(temp_workspace):
    ...
    obj = MemoryObject(id="mem_leak_001", object_type="report", content={"leak_rate": "high"}, tags=["fuel", "engine"], confidence=ConfidenceLevel.HYPOTHESIS)
    runtime.memory.store(obj)
    runtime.decisions.record_decision(decision_type=DecisionType.TECHNICAL, description="Replace engine B gasket", rationale="Seal fuel leakage", evidence=["mem_leak_001"], decision_id="dec_replace_gasket")
...
def test_self_verification_integrity(temp_workspace):
    ...
    runtime.decisions.record_decision(decision_type=DecisionType.PROCESS, description="A decision", rationale="Just because", evidence=["ghost_evidence_id"], decision_id="dec_with_ghost")
...
def test_cli_continuity_bridge_commands(temp_workspace):
    ...
    payload_data = {"session_id": "cli_session_123", "objective": "Test SAGE CLI objective", "task": "Test SAGE CLI task", "memories": [], "decisions": []}
...
def test_ai_clients_and_tools_bridge_routing(temp_workspace):
    ...
    runtime.set_task("Verify unified bridge routing")
    chatgpt = ChatGPTClient(runtime)
    gemini = GeminiJulesClient(runtime)
    tool_mgr = ToolIntegrationManager(runtime)
    chatgpt_req = AIQueryRequest(prompt="Explain the Continuity Bridge architecture", response_override="Deterministic ChatGPT bridge test response")
    ...
    gh_event = GitHubEvent(event_type="commit", repository="test-repo", author="Jules", payload={"message": "Initial design"})
    tool_mgr.index_github_event(gh_event)
    memories = runtime.memory.list_all()
    gh_mems = [m for m in memories if "github" in m.tags]
    assert len(gh_mems) == 1
    artifact = GoogleWorkspaceArtifact(doc_id="arch_spec_1", title="Architecture Specification Doc", doc_type="doc", last_modified_by="Jules", url="https://drive.google.com/doc1")
...
def test_live_continuity_path_e2e(temp_workspace):
    runtime = SageRuntime(str(temp_workspace))
    payload_data = {"session_id": "activation_test_session_001", "objective": "Verify SAGE Final Activation Checkpoint", "task": "Test live continuity path end-to-end", "memories": [{"id": "mem_activation_001", "object_type": "rule", "content": {"title": "Activation Rules", "description": "SAGE is fully operational under python 3.12+ and Pydantic v2", "archive": True}, "tags": ["activation", "verify"], "confidence": "validated"}], "decisions": [{"id": "dec_activation_001", "decision_type": "technical", "description": "Approve final activation", "rationale": "All verification criteria are fully met on the repository-side", "evidence": ["mem_activation_001"]}]}
    result = runtime.ingest_session_payload(ExternalSessionPayload(**payload_data))
    assert result["status"] == "success"
...
def test_ai_query_response_override(temp_workspace):
    runtime = SageRuntime(str(temp_workspace))
    runtime.set_objective("Test AI response override")
    runtime.set_task("Verify governed override path")
    chatgpt = ChatGPTClient(runtime)
    gemini = GeminiJulesClient(runtime)
