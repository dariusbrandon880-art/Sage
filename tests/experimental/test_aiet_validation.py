...
from sage.experimental.aiet import AIETBlindScenario, AIETExternalClient, AIETExternalClientError, AIETExternalHarnessServer, AIETExternalServerError, AIETMetricsCalculator, AIETMissionRunner, AIETProviderAdapter, AIETProviderConfig, AIETValidationReceipt, FailurePerturbation
...
def _contract(mission_id="aiet_test"):
    return {"schema_version": "1.0", "mission_id": mission_id, "intent": "AIET external harness launch-readiness verification", "authority_boundary": {"allowed_paths": ["sage/experimental/aiet/**"], "prohibited_paths": ["sage/runtime/**", "sage/core/**"]}, "completion_criteria": {"provenance_required": True}}
...
def test_perturbation_injector():
    ...
    changed, logs = AIETPerturbationInjector([p]).apply({"x": "value"})
...
def test_metrics_calculator():
    b = FitnessVector(mission_value=.6, correctness=.6, repeatability=.6, evidence_quality=.6, recovery=.6, generalization=.6, cost=1)
    c = FitnessVector(mission_value=.9, correctness=.95, repeatability=.9, evidence_quality=.9, recovery=.9, generalization=.9, cost=.8)
    p = FitnessVector(mission_value=.8, correctness=.85, repeatability=.8, evidence_quality=.8, recovery=.85, generalization=.8, cost=.9)
    t = FitnessVector(mission_value=.85, correctness=.9, repeatability=.85, evidence_quality=.85, recovery=.85, generalization=.85, cost=.8)
...
def test_runner_fixture_fails_closed():
    ...
    receipt = AIETMissionRunner().run_validation_flight(mission_contract=MissionContract.from_mapping(_contract()), scenarios=[scenario], baseline_executor=lambda _: {"status": "ok"}, candidate_executor=lambda _: {"status": "ok"}, perturbations=[], baseline_technique_id="b", candidate_technique_id="c", execution_mode="fixture")
...
def test_provider_invocation_hits_real_http_endpoint(monkeypatch):
    class Handler(BaseHTTPRequestHandler):
        def do_POST(self):
            length = int(self.headers["Content-Length"]); body = json.loads(self.rfile.read(length)); assert body["model"] == "gpt-test"
            payload = {"id": "real-http-1", "choices": [{"message": {"content": "live provider response"}}]}; data = json.dumps(payload).encode()
            self.send_response(200); self.send_header("Content-Type", "application/json"); self.send_header("Content-Length", str(len(data))); self.end_headers(); self.wfile.write(data)
        def log_message(self, *_): return
    server = ThreadingHTTPServer(("127.0.0.1", 0), Handler); threading.Thread(target=server.serve_forever, daemon=True).start()
    monkeypatch.setenv("OPENAI_API_KEY", "test-key"); monkeypatch.setenv("OPENAI_BASE_URL", f"http://127.0.0.1:{server.server_port}")
    try:
        ...
        assert result["result"] == "live provider response" and result["raw_provider_response"]["id"] == "real-http-1" and result["synthesis_digest"] and "mission_value" not in result
...
def test_actual_out_of_band_disruption_and_recovery(monkeypatch):
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            payload = {"id": "recovered", "choices": [{"message": {"content": "recovered provider response"}}]}; data = json.dumps(payload).encode(); self.send_response(200); self.send_header("Content-Type", "application/json"); self.send_header("Content-Length", str(len(data))); self.end_headers(); self.wfile.write(data)
        def log_message(self, *_): return
    server = ThreadingHTTPServer(("127.0.0.1", 0), Handler); threading.Thread(target=server.serve_forever, daemon=True).start()
    monkeypatch.setenv("OPENAI_API_KEY", "test-key"); monkeypatch.setenv("OPENAI_BASE_URL", f"http://127.0.0.1:{server.server_port}"); monkeypatch.setenv("SAGE_AIET_DISRUPTION_URL", f"http://127.0.0.1:{server.server_port}/fail")
    try:
        ...
        assert result["disruption_encountered"] is True and "HTTP 503" in result["disruption_error"] and result["result"] == "recovered provider response"
    finally: server.shutdown()
...
def test_receipt_tamper_and_fixture_rejection():
    ...
    with pytest.raises(AIETExternalClientError, match="RECEIPT_HASH_MISMATCH"): AIETExternalClient.validate_remote_receipt(data)
    ...
    with pytest.raises(AIETExternalClientError, match="INVALID_EXTERNAL_RECEIPT"): AIETExternalClient.validate_remote_receipt(fixture)
