...
def test_out_of_sample_data_separator():
    separator = OutOfSampleDataSeparator(oos_cutoff_timestamp_utc="2026-08-20T00:00:00Z")

    valid_obs = RealSportsEventObservation(
        event_id="ev_oos_valid",
        sport="baseball", league="mlb", home_team="NYY", away_team="BOS",
        event_start_time_utc="2026-08-22T20:00:00Z", observation_timestamp_utc="2026-08-22T18:00:00Z",
        source_name="MLB", source_url="https://statsapi.mlb.com", market_name="Moneyline",
        observed_odds={"home": -110}, event_status="SCHEDULED"
    )

    invalid_obs = RealSportsEventObservation(
        event_id="ev_oos_invalid",
        sport="baseball", league="mlb", home_team="LAD", away_team="SDP",
        event_start_time_utc="2026-08-18T20:00:00Z", observation_timestamp_utc="2026-08-18T18:00:00Z",
        source_name="MLB", source_url="https://statsapi.mlb.com", market_name="Moneyline",
        observed_odds={"home": -120}, event_status="SCHEDULED"
    )

    res = separator.validate_and_partition([valid_obs, invalid_obs])

    assert res["total_evaluated"] == 2
    assert res["oos_valid_count"] == 1
    assert res["falsified_rejections_count"] == 1
    assert res["oos_observations"][0]["event_id"] == "ev_oos_valid"
    assert res["falsified_rejections"][0]["event_id"] == "ev_oos_invalid"
    assert "TEMPORAL_LEAKAGE_REJECTED" in res["falsified_rejections"][0]["reason"]
