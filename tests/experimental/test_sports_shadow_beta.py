...
def test_external_signal_input_separation():
    ...
    ext_sig = ExternalSignalInput(
        ...
        source_url="http://public.consensus.api",
...
def test_concurrent_sports_prediction_engine():
    ...
    obs1 = RealSportsEventObservation(
        ...
        observed_odds={"status": "ODDS_UNAVAILABLE"}, event_status="Scheduled", pre_event=True
    )
    ...
    def dummy_model(obs):
        return f"{obs.home_team} Moneyline", 0.6000, "Dummy rationale", "ODDS_UNAVAILABLE", None
...
def test_clv_and_log_loss_diagnostics():
    ...
    assert clv_val is not None
    assert clv_val > 0.0  # Model prob higher than closing line prob indicates positive CLV

    # When closing line is unavailable, calculate_clv returns None
    assert calculate_clv(0.5800, None) is None
...
def test_parlay_leg_decomposition_learning():
    ...
    obs_leg1 = RealSportsEventObservation(
        ...
        event_start_time_utc="2026-08-30T20:00:00Z", observation_timestamp_utc="2026-08-30T19:00:00Z",
        source_name="API", source_url="http", market_name="Moneyline", observed_odds={}, event_status="Final", pre_event=True
    )
    ...
    pred_leg1 = LockedResearchPrediction(
        ...
        model_predicted_probability=0.58, lock_timestamp_utc="2026-08-30T19:00:00Z", model_state_rationale="Leg 1", pre_event=True
    )
    pred_leg2 = LockedResearchPrediction(
        ...
        model_predicted_probability=0.58, lock_timestamp_utc="2026-08-30T19:00:00Z", model_state_rationale="Leg 2", pre_event=True
    )

    obs_parlay = RealSportsEventObservation(
        event_id="e_parlay", sport="baseball", league="mlb", home_team="Multi", away_team="Multi",
        event_start_time_utc="2026-08-30T20:00:00Z", observation_timestamp_utc="2026-08-30T19:00:00Z",
        source_name="API", source_url="http", market_name="Parlay", observed_odds={}, event_status="Final", pre_event=True
    )
    pred_parlay = LockedResearchPrediction(
        ...
        model_predicted_probability=0.33, lock_timestamp_utc="2026-08-30T19:00:00Z", model_state_rationale="Parlay", pre_event=True,
...
def test_anti_short_term_batch_model_promotion_governance():
    ...
    for i in range(2):
        pred = LockedResearchPrediction(
            ...
            selected_prediction="NYY Moneyline", odds_at_lock="-110", implied_probability=0.52,
            model_predicted_probability=0.60, lock_timestamp_utc="2026-08-30T19:00:00Z", model_state_rationale="Short", pre_event=True
        )
