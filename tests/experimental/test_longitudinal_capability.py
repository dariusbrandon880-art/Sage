...
def _observation(system, mission_id, success=True, recovery=False, regression=False, session="s1", retained=True):
    return FlightObservation(system=system, mission_id=mission_id, session_id=session, success=success, recovered_after_failure=recovery, evidence_complete=True, provenance_preserved=True, unauthorized_transition_blocked=True, continuity_intact=True, retained_across_sessions=retained, learning_candidate_quality=0.9, elapsed_seconds=10.0, cost_units=1.0, regression_detected=regression)


def _suite(system, successes):
    return [_observation(system, f"m{i}", success=success, recovery=(i == 2 and system == "sage"), session=f"s{i}") for i, success in enumerate(successes, 1)]
...
def test_baseline_comparison_can_prove_positive_result():
    receipt = LongitudinalCapabilityEvaluator(_plan()).evaluate(_suite("baseline", [True, False, False, False]), _suite("sage", [True, True, True, True]))
...
def test_missing_mission_fails_closed_before_verdict():
    try:
        LongitudinalCapabilityEvaluator(_plan()).evaluate(_suite("baseline", [True, True, True, True]), _suite("sage", [True, True, True]))
...
def test_retention_failure_is_required_only_for_reuse_missions():
    sage = _suite("sage", [True, True, True, True])
    sage[2] = FlightObservation(**{**sage[2].__dict__, "retained_across_sessions": False})
    receipt = LongitudinalCapabilityEvaluator(_plan()).evaluate(_suite("baseline", [True, False, False, False]), sage)
    assert receipt.verdict is CapabilityVerdict.PASS


def test_required_retention_failure_is_negative_result():
    sage = _suite("sage", [True, True, True, True])
...
def test_duplicate_observation_is_rejected():
    ...
    try:
        LongitudinalCapabilityEvaluator(_plan()).evaluate(baseline, _suite("sage", [True, True, True, True]))
    except ValueError as exc:
...
def test_invalid_input_does_not_consume_evaluation():
    evaluator = LongitudinalCapabilityEvaluator(_plan())
    try:
        evaluator.evaluate(_suite("baseline", [True, True, True, True]), _suite("sage", [True, True, True]))
    except ValueError:
        pass
    else:
        raise AssertionError("expected validation failure")
    receipt = evaluator.evaluate(_suite("baseline", [True, False, False, False]), _suite("sage", [True, True, True, True]))
    assert receipt.verdict is CapabilityVerdict.PASS
