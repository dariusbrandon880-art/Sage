...
def market(
    ...
    return MarketSnapshot(
        ...
        market=market_name,
        market_type=market_type or market_name,
...
def test_daily_engine_reaches_50_without_duplicate_prediction_identity():
    snapshots = [
        market(f"mlb-{i}", "MLB", "moneyline", {"home": 2.0, "away": 2.0}) for i in range(20)
    ]
    snapshots += [
        market(f"nba-{i}", "NBA", "spread", {"home": 1.9, "away": 1.9}, "spread", -3.5)
        for i in range(20)
    ]
    snapshots += [
        market(f"nfl-{i}", "NFL", "total", {"over": 1.9, "under": 1.9}, "total", 45.5)
        for i in range(20)
    ]
    snapshots += [
        market(f"nhl-{i}", "NHL", "moneyline", {"home": 2.0, "away": 2.0}) for i in range(20)
    ]
    portfolio = DailySportsPortfolioEngine(target=50, parlay_share=0.30).build(
        snapshots, "daily-2026-09-05"
    )
    ...
    assert (
        len({record.selection for record in portfolio.records if record.is_parlay})
        == portfolio.parlay_count
    )
...
def test_daily_engine_builds_only_three_to_seven_leg_parlays():
    ...
    portfolio = DailySportsPortfolioEngine(target=20, parlay_share=0.50).build(
...
def test_daily_engine_supports_soccer_and_tennis():
    snapshots = [
        market(f"soccer-{i}", "SOCCER", "moneyline", {"home": 2.1, "draw": 3.2, "away": 3.4})
        for i in range(10)
    ]
    snapshots += [
        market(f"tennis-{i}", "TENNIS", "moneyline", {"player1": 1.8, "player2": 2.1})
        for i in range(10)
    ]
    portfolio = DailySportsPortfolioEngine(target=20, parlay_share=0.25).build(
        snapshots, "daily-multi-sport"
    )
...
def test_daily_engine_rejects_unknown_sports():
    with pytest.raises(ValueError, match="UNSUPPORTED_SPORTS"):
        DailySportsPortfolioEngine(target=1).build(
            [market("cricket-1", "CRICKET", "moneyline", {"home": 2.0, "away": 2.0})],
            "daily-invalid",
        )
