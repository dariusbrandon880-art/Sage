...
def market(event_id: str, sport: str, market_name: str, prices: dict[str, float]) -> MarketSnapshot:
...
def test_daily_engine_reaches_50_without_duplicate_prediction_identity():
    snapshots = [
        market(f"mlb-{i}", "MLB", "moneyline", {"home": 2.0, "away": 2.0})
        for i in range(20)
    ]
    snapshots += [
        market(f"nba-{i}", "NBA", "spread", {"home": 1.9, "away": 1.9})
        for i in range(20)
    ]
    snapshots += [
        market(f"nfl-{i}", "NFL", "total", {"over": 1.9, "under": 1.9})
        for i in range(20)
    ]
    snapshots += [
        market(f"nhl-{i}", "NHL", "moneyline", {"home": 2.0, "away": 2.0})
        for i in range(20)
    ]
    ...
    assert len({
        (record.event_id, record.market, record.selection, record.model_version, record.observed_at_utc)
...
def test_daily_engine_builds_only_three_to_six_leg_parlays():
    snapshots = [
        market(f"event-{i}", "NBA", "moneyline", {"home": 2.0, "away": 2.0})
        for i in range(10)
    ]
...
def test_daily_engine_rejects_unknown_sports():
    with pytest.raises(ValueError, match="UNSUPPORTED_SPORTS"):
        DailySportsPortfolioEngine(target=1).build(
            [market("soccer-1", "MLS", "moneyline", {"home": 2.0, "away": 2.0})],
            "daily-invalid",
        )
